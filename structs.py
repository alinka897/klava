class Key():
    ...
    def __init__(self, code: int, char: str, /, alt=False):
        """функция init  определяет диапазон нужных клавиш
           и зажат ли альт
        Аргуmenты:
            code:int : номер(код) клавши
            char:str : значение клавиши(стринг)
            alt : зажат ли альт
        """
...
class Layout():
    ...
    Аргуmenты:
        tr=... : значения для верхнейшего ряда
        ur=... : значения для верхнего ряда
        hr=... : значения для домашнего ряда
        lr=... : значения для нижнего ряда
    ...
    def extract_keys(self, alts: dict):
        """Получает словари с клавишами раскладки и
           альтовыми клавишами
        аргуmenты:
            self.[нужное вставить] : из какой строки
            range([нужное вставить],[нужное вставить два]) : какая клавиша
        """
    ...
    """Считает штрафы для рук и пальцев в строке
       (в том числе зажат ли шыфт, ибо жто штраф(der Shtraffe))
    Аргуmenты:
        line: str : текст в строке
    """
